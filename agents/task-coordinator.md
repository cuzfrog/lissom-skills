...
description: >
  ...
  structured verdict consumed by task-auto or a parent coordinator.
tools: Agent, Read
model: opus
---
...
1. Invoke `task-research` with task ID and mode. Verify `Research.md` exists; retry once on missing, then emit `DONE: FAIL -- task-research artifact missing`.
2. Invoke `task-plan` with task ID. Verify `Plan.md` exists; retry once on missing before emitting `DONE: FAIL -- task-plan artifact missing`.
3. Invoke `task-impl` with task ID. Verify `Impl-summary.md` exists; retry once on missing.
4. Invoke `task-review` with task ID. Verify `Review.md` exists; retry once on missing.
...
Up to 3 cycles:
1. Invoke `task-plan` with task ID, `fix_cycle=M`, and instruction: "Fix cycle <M>: read Review.md and produce fix step files."
2. Invoke `task-impl` with task ID.
3. Invoke `task-review` with task ID.
...
Up to 3 attempts:
1. Invoke `task-research` with task ID and `mode=auto`.
2. Re-invoke `task-plan` with task ID.
3. If `task-plan` no longer escalates → return to the calling phase.

After 3 attempts still escalating, pause and relay the conflict to the user. Resume only on explicit user instruction.

## Multi-task path

1. Invoke `task-dependency-researcher` with the full list of task IDs. Relay any user-facing questions from it before proceeding. Read each `Dependency.md` to get final execution order.
2. **Serialised research**: For each task in execution order, invoke `task-research` with task ID and mode. Verify `Research.md` exists after each. If still missing after the skill's internal retry, mark the task `"failed"` in the result map and continue with the remaining tasks. Complete all research before any plan/impl/review begins.
3. **Sequential execution**: Maintain a result map `{task_id: "pending" | "success" | "failed" | "blocked"}`.
   ...
   c. Otherwise, invoke a child `task-coordinator` (single-task mode) with task ID and mode. Await its verdict before proceeding to the next task.
...
- Any invocation of `task-plan` may return `ESCALATE: stale-conflict`. Always respond by executing the conflict re-research loop, then continuing the current phase. Count the attempt against the current phase's retry or cycle limit.
...
- Downstream agents (`task-researcher`, `task-planner`) read `Dependency.md` directly; the coordinator does not forward its contents.
